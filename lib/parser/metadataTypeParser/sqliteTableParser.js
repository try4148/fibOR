...
let sqliteTableValidator = require('../.././validator/metadataTypeValidator/sqliteTableValidator.js');
let sqliteTableFormatter = require('../.././formatter/metadataTypeFormatter/sqliteTableFormatter.js');
let filterValidator = require('../.././validator/filterValidator.js');

let sqliteTableParser = {
	'parseGetSql': function(objHttp, metadata, verifyUri) {
		...
		let validator = sqliteTableValidator.verifyUriParams(uri, verifyUri);
		if (!validator['result']) {
				...
				if (type !== 'function') {
					...
					try {
						if (r[f] === undefined) {
							...
							row[f] = sqliteTableFormatter.formatValueTypeRsOut(r[f], type, pattern);
						}
					} catch (e) {
						//console.warn(e);
					}
...
//console.warn(sql);
	...
	'parseIndex': function(objHttp, metadata) { //获取index
		...
		let validator = sqliteTableValidator.verifyIndex(metadata, index);
		if (!validator['result']) {
			objReturn['result'] = false;
			objReturn['reMsg'] = objReturn['reMsg'] + validator['reMsg'];
			return validator;
		}
		objReturn['reMsg'] = sqliteTableFormatter.formatIndex(metadata, index);
		return objReturn;
	},
	...
	'parseFilter': function(filter, metadata, KeyWords) {
		...
		let validator = sqliteTableValidator.verifyAstTreeLeafType(astTree, metadata['content']['fields'], filterParser.defaultKeyWords);
		if (!validator['result']) {
			validator['reMsg'] = '解析filter:' + validator['reMsg'];
			return validator;
		}
		filter = sqliteTableFormatter.formatAstTree(astTree, metadata, filterParser.defaultKeyWords);
	...
	'parseSelect': function(select, metadata, pMeta) {
		...
		for (let a in arrSelect) {
			let validator = sqliteTableValidator.verifyParamsFields(metadata['content']['fields'], arrSelect[a].trim(), 'select');
			if (!validator['result']) {
		...
		objReturn['reMsg'] = sqliteTableFormatter.formatSelect(arrSelect, metadata['config']['mapping']['fields']);
		return objReturn;
	},
	'parseOrderby': function(orderby, metadata) {
		...
		for (let o in orderbyArray) {
			...
			validator = sqliteTableValidator.verifyParamsFields(metadata['content']['fields'], oaaf, 'select');
			if (!validator['result']) {
		...
		objReturn['reMsg'] = sqliteTableFormatter.formatOrderby(pOrderby['reMsg'], metadata['config']['mapping']['fields']);
		return objReturn;
	},
	...
	'parseCount': function(count, metadata) {
		...
		for (let c in countArray) {
			...
			let validator = sqliteTableValidator.verifyParamsFields(metadata['content']['fields'], countFields, 'select');
			if (!validator['result']) {
		...
		for (let p in pCount) {
			...
			if (tCount === 'true' || tCount === 'null') {
				rCount = rCount === '' ? "count(" + (tCount === 'true' ? '1' : 'null') + ") as '" + tCount + "'" : rCount + ",count(" + (tCount === 'true' ? '1' : 'null') + ") as '" + tCount + "'";
			} else {
	...
	'parseBody': function(objHttp, metadata) {
		...
		if (objHttp.method === 'POST') {
			...
			for (let r in body) {
				...
				for (let f in body[r]) {
					let validator = sqliteTableValidator.verifyParamsFields(metadata['content']['fields'], f, 'insert');
					if (!validator['result']) {
					...
					let pattern = metadata['content']['fields'][f]['pattern'];
					value = sqliteTableFormatter.formatValueTypeRsIn(value, type, pattern);
		...
		if (objHttp.method === 'PUT') {
			let objBody = new Object();
			for (let f in body) {
				let validator = sqliteTableValidator.verifyParamsFields(metadata['content']['fields'], f, 'update');
				if (!validator['result']) {
	...
	'parseMetadata': function(uriMetadata, metadata) {
		...
		if (metadata['content']['fields'][uriMetadata]['type'] === 'binary' && metadata['content']['fields'][uriMetadata]['select']) {
...
module.exports = sqliteTableParser;