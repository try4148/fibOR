 ...
 * @author 刀叉4148
 *----------------------------------
 * 元数据类型sqliteTable的通用处理程序
 */

'use strict';

let sqliteTableParser = require('../.././parser/metadataTypeParser/sqliteTableParser.js');

let sqliteTableActor = {
	'GET': function(objHttp, metadata, dbPool, contentType) {
		...
		let parse = sqliteTableParser.parseGetSql(objHttp, metadata, 'select,filter');
		if (!parse['result']) {
			return parse;
		}
		...
		if (!parse['reMsg']['metaIsBinary']) {
			...
			try {
				...
				if (parse['reMsg']['metaExt']) {
					...
					sqlResult['result'] = sqliteTableParser.parseGetRs(rs, metadata);
				}
	...
	'PUT': function(objHttp, metadata, dbPool, contentType) {
		...
		let parse = sqliteTableParser.parsePutSql(objHttp, metadata, 'filter');
		if (!parse['result']) {
			return parse;
		}
	...
	'POST': function(objHttp, metadata, dbPool, contentType) {
		...
		let parse = sqliteTableParser.parsePostSql(objHttp, metadata);
		if (!parse['result']) {
			return parse;
		}
	...
	'DELETE': function(objHttp, metadata, dbPool, contentType) {
		...
		let parse = sqliteTableParser.parseDeleteSql(objHttp, metadata, 'filter');
		if (!parse['result']) {
			return parse;
		}
...
module.exports = sqliteTableActor;