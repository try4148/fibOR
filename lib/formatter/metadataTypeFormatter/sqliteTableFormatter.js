...
let mysqlTableFormatter = {
				...
				for (let i = 1; i < 3; i++) {
					if (typeof(astObject['l' + i]) === 'string') {
						if (metadata['content']['fields'].hasOwnProperty(astObject['l' + i])) { //字段	
							...
							astObject['l' + i] = astObject['l' + i] === 'null' ? 'null' : metadataValueTypeFormatter.formatValueType(astObject['l' + i], type, pattern, false);
						}
					} else { //函数
						astObject['l' + i] = this.formatAstTreeLeafType(astObject['l' + i], metadata, keyWords);
					}
			...
			if (keyWords['func'].hasOwnProperty(k)) {
				...
				for (let i = 0; i < arrType.length; i++) {
					if (metadata['content']['fields'].hasOwnProperty(astObject['l' + (i + 1)])) { //字段
						...
						let type = arrType[i];
						let pattern = '';
						astObject['l' + (i + 1)] = astObject['l' + (i + 1)] === 'null' ? 'null' : metadataValueTypeFormatter.formatValueType(astObject['l' + (i + 1)], type, pattern, false);
					}
				}
			}
		}
	...
	'formatIndex': function(metadata, index) {
		...
		for (let i in fieldsArray) {
			...
			let dataValue = metadataValueTypeFormatter.formatValueType(index['r' + i], dataType, dataPattern, false);
	...
	'formatValueTypeRsOut': function(dataValue, dataType, dataPattern) {
		...
		if (dataType === 'datetime') {
			...
			if (!isNaN(dataValue)) {
				dataValue = dataValue.toString() + '0000000000000';
				dataValue = parseInt(dataValue.substr(0, 13));
			}
			let zoneOffset = 0;
			if(dataValue instanceof Date){
				zoneOffset = dataValue.getTimezoneOffset() * -1;
			}
			dataValue = (new Date(dataValue)).toJSON();
			let m = moment(dataValue);
			return m.utcOffset(zoneOffset).format(dataPattern);
		}
		if (dataType === 'binary') {
			if (dataPattern === 'hex') {
				let hex = require('hex');
