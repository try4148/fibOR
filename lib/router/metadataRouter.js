	...
	'createMetadataRouting':function(index){
		...
		for (let keysIndex in arrMetadataCacheKeys) {//append资源到routing
			...
			routing.append(routeReg, function(objHttp) {
				for (let q in objHttp.query) { //通过加入method关键词兼容不支持put，delete方法的浏览器
					if (q.toLowerCase()=== '$method') {
						objHttp.method = encoding.decodeURI(objHttp.query[q]).trim().toUpperCase();
					}
				}
