 ...
 * @author 刀叉4148
 *----------------------------------
 * 对元数据type为mysqlTable的content部分进行验证
 */
...
let mysqlTableValidator = {
	'verify': function(metadata, metadataPool) {
		...
		if (metadata['config']['resource'].substr(0,metadata['config']['resource'].indexOf(':')) !== 'mysql') {
			objReturn['result'] = false;
			objReturn['reMsg'] = '元数据resource项不是正确的mysql连接串';
			objReturn['reCode'] = 0;
			return objReturn;
		}
		...
		if (metadataPool.getType(metadata['config']['resource']) !== 'mysql') {
			objReturn['result'] = false;
		...
		let verifySql = "select count(*) tabcnt from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA = database() and TABLE_NAME='" + tableName + "'";
		try {
			...
			objReturn['result'] = false;
			objReturn['reMsg'] = '无法获取mysqlTable资源';
			objReturn['reCode'] = 0;
			return objReturn;
		}
	...
	'verifyFieldsIsExists': function(dbConn, tableName, fieldsName) {
		...
		objReturn['reMsg'] = '';
		objReturn['reCode'] = 0;
		let verifySql = 'Describe ' + tableName + ' ' + fieldsName;
		let verifySqlResult = dbConn.execute(verifySql);
		if (verifySqlResult.length !== 1) {
			objReturn['result'] = false;
...
module.exports = mysqlTableValidator;